<script lang="ts">
  ...
  import { getAudioData, getFilterData, getFramesData } from "waveform-path";
  ...
  let chunks: Float32Array[] = new Array(zoom).fill([]);
  ...
  function calculateChunks() {
    ...
    const total = frameData[0].length;
    for (let i = 0; i < zoom; i++) {
      chunks[i] = frameData[0].slice(
        (total / zoom) * i,
        (total / zoom) * (i + 1)
      );
    }
